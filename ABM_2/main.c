...
int cargarDescripcion (eSectores sectores[],int tamSector, int idSector, int idSector);
...
void mostrarEmpleado(eEmpleado emp, eSector sectores [i], int tam)
{
    char descrioncion [20];
    cargarDescripcion (sectores,cadSector,descripcion);
    printf("%d %s %c %.2f \n\n", emp.legajo, emp.nombre, emp.sexo, emp.sueldo);

}


void mostrarEmpleados(eEmpleado nomina[], int tam,eSector sectores[i])
{
    for(int i=0; i< tam; i++)
    {
